...
VertexToPixel main( VertexShaderInput input )
{
	...
	float3x3 worldNoTran = (float3x3)world;
	output.normal = normalize(mul(input.normal, worldNoTran));
	output.tangent = normalize(mul(input.tangent, worldNoTran));

	// copy the uv
	output.uv = input.uv;

	// calculate the world positioning
